...
import { Ban, Calendar, Clock, BadgeIcon as IdCard, Phone, User, Scissors, Check, Sparkles } from "lucide-react"
...
const FemininoModernoAgendamento = () => {
        ...
        <BarbeariaStep>
          <div className="flex flex-col items-center mb-6">
            ...
            <div className="flex items-center">
              ...
              <Sparkles className="mx-2" size={14} style={{ color: theme.colors.primary }} />
              <div className="h-[1px] w-8" style={{ backgroundColor: theme.colors.primary }}></div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pb-10">
            {list.map((item) => (
              <motion.div
                ...
                style={{
                  background: isSelecionado(item.id)
                    ? `linear-gradient(135deg, ${theme.colors.primary}, ${theme.colors.secondary})`
                    : theme.colors.cardBackground,
                  borderRadius: theme.borderRadius.large,
                  boxShadow: isSelecionado(item.id) ? `0 10px 25px -5px ${theme.colors.primary}40` : "",
                }}
              ...
              >
                <div className="relative h-40">
                  ...
                  {isSelecionado(item.id) && (
                    <div className="absolute top-2 right-2 rounded-full p-1" style={{ backgroundColor: "white" }}>
                  ...
                  <div className="absolute bottom-0 left-0 p-3">
                    ...
                    <p
                      ...
                      style={{ color: isSelecionado(item.id) ? "white" : theme.colors.primary }}
                    >
        ...
        <BarbeariaStep>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pb-10">
            <motion.div
              ...
              style={{
                background: isBarbeiroSelecionado(0)
                  ? `linear-gradient(135deg, ${theme.colors.primary}, ${theme.colors.secondary})`
                  : theme.colors.cardBackground,
                borderRadius: theme.borderRadius.large,
                boxShadow: isBarbeiroSelecionado(0) ? `0 10px 25px -5px ${theme.colors.primary}40` : "",
              }}
              whileHover={{ y: -5, transition: { duration: 0.3 } }}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <Ban
                size={48}
                style={{ color: isBarbeiroSelecionado(0) ? "white" : theme.colors.textSecondary }}
                className="mb-2"
              />
              <h4
                className="text-lg font-semibold text-center"
                style={{ color: isBarbeiroSelecionado(0) ? "white" : theme.colors.text }}
              >
                Sem preferência
              </h4>
            ...
            {barbeiros.map((item) => (
              <motion.div
                ...
                className={`cursor-pointer overflow-hidden shadow-lg relative`}
                style={{
                  background: isBarbeiroSelecionado(item.id)
                    ? `linear-gradient(135deg, ${theme.colors.primary}, ${theme.colors.secondary})`
                    : theme.colors.cardBackground,
                  borderRadius: theme.borderRadius.large,
                  boxShadow: isBarbeiroSelecionado(item.id) ? `0 10px 25px -5px ${theme.colors.primary}40` : "",
                }}
                whileHover={{ y: -5, transition: { duration: 0.3 } }}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <div className="flex flex-col mt-6">
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-5 gap-4">
                    {[
                      ...
                      <motion.div
                        key={time}
                        className={`text-center py-2 px-1 rounded-md cursor-pointer`}
                        style={{
                          background:
                          ...
                          boxShadow:
                            (typeof novo.horaAgendamento === "string" ? novo.horaAgendamento : "") === time
                              ? `0 5px 15px -3px ${theme.colors.primary}40`
                              : "",
                        }}
...
export default FemininoModernoAgendamento
